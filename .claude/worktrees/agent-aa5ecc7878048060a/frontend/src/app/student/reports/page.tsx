...
function buildSubtitleGroups(
...
): { subtitleId: string; subtitleLabel: string; items: { def: StudentReportItemResponse; checked: boolean | null; textValue: string | null }[] }[] {
  const itemsMap: Record<string, { checked: boolean | null; textValue: string | null }> = {};
  detail?.items.forEach((item) => {
    itemsMap[item.reportItemDefinitionId] = { checked: item.checked, textValue: item.textValue };
  });

  const groups: { subtitleId: string; subtitleLabel: string; items: { def: StudentReportItemResponse; checked: boolean | null; textValue: string | null }[] }[] = [];
  for (const def of defs) {
    ...
    const values = itemsMap[def.id] ?? { checked: null, textValue: null };
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setDefsByDate(Object.fromEntries(entries.map((e) => [e.date, e.defs])));
        ...
        {visibleReports.length === 0 ? (
          ...
          visibleReports.map((report) => {
            ...
            groups.forEach((group) => {
              group.items.forEach(({ def, checked, textValue }) => {
                if (def.itemType === "CHECKBOX") {
                  bodySummaryParts.push(`${checked ? "✓" : "☐"}${def.label}`);
                } else {
                  bodySummaryParts.push(`${def.label}: ${textValue || "未入力"}`);
                }
              });
            });
                      ...
                      {groups.map((group) => (
                        <div key={group.subtitleId}>
                          ...
                          <div className="stack-sm" style={{ paddingLeft: "14px" }}>
                            {group.items.map(({ def, checked, textValue }) =>
                              def.itemType === "CHECKBOX" ? (
                                ...
                                <div key={def.id} style={{ fontSize: "0.8rem" }}>
                                  <span className="muted">{def.label}: </span>
                                  <span>{textValue || "（未入力）"}</span>
                                </div>
                              )
                            )}
                          </div>
