...
interface ReportItemGroupsApi {
  ...
  createItem(
    ...
    groupId: string,
    subtitleId: string,
    req: { label: string; itemType: ReportItemType }
  ...
  reorderItems(
    ...
    groupId: string,
    subtitleId: string,
    req: { itemType: ReportItemType; orderedIds: string[] }
  ): Promise<void>;
}
  ...
  return (
    <div style={{ marginTop: 12 }}>
      ...
      <div className="row" style={{ marginTop: 8 }}>
        <input
          className="input"
          placeholder={itemType === "CHECKBOX" ? "例: 朝勉強した" : "例: 今日の反省"}
          value={newLabel}
...
export function ReportItemsManager({
  ...
  const [newTextLabel, setNewTextLabel] = useState<Record<string, string>>({});
  ...
  async function handleCreateItem(subtitleId: string, itemType: ReportItemType) {
    if (!selectedGroupId) return;
    const label = itemType === "CHECKBOX" ? newCheckboxLabel[subtitleId] : newTextLabel[subtitleId];
    if (!label?.trim()) return;
    setSubmitting(true);
    setError(null);
    try {
      const created = await reportItemGroups.createItem(selectedStudentId, selectedGroupId, subtitleId, {
        label,
        itemType,
      });
      ...
      if (itemType === "CHECKBOX") {
        ...
        setNewTextLabel((prev) => ({ ...prev, [subtitleId]: "" }));
      }
    } catch (err) {
  ...
  async function handleReorder(subtitleId: string, itemType: ReportItemType, orderedIds: string[]) {
    if (!selectedGroupId) return;
    const current = itemsBySubtitle[subtitleId] ?? [];
    const byId = new Map(current.map((i) => [i.id, i]));
    const reorderedOfType = orderedIds.map((id) => byId.get(id)!).filter(Boolean);
    let cursor = 0;
    const merged = current.map((i) => (i.itemType === itemType ? reorderedOfType[cursor++] : i));
    setItemsBySubtitle((prev) => ({ ...prev, [subtitleId]: merged }));
    try {
      await reportItemGroups.reorderItems(selectedStudentId, selectedGroupId, subtitleId, {
        itemType,
        orderedIds,
      });
    } catch (err) {
                          ...
                          {loadingItemsSubtitleId === subtitle.id ? (
                            ...
                            <>
                              <ItemTypeList
                                title="チェック項目"
                                itemType="CHECKBOX"
                                items={(itemsBySubtitle[subtitle.id] ?? []).filter((i) => i.itemType === "CHECKBOX")}
                                submitting={submitting}
                                onReorder={(orderedIds) => handleReorder(subtitle.id, "CHECKBOX", orderedIds)}
                                ...
                                onCreate={() => handleCreateItem(subtitle.id, "CHECKBOX")}
                              />
                              <ItemTypeList
                                title="記述項目"
                                itemType="TEXT"
                                items={(itemsBySubtitle[subtitle.id] ?? []).filter((i) => i.itemType === "TEXT")}
                                submitting={submitting}
                                onReorder={(orderedIds) => handleReorder(subtitle.id, "TEXT", orderedIds)}
                                onStartEdit={handleStartEditItem}
                                onDelete={(item) => handleDeleteItem(subtitle.id, item)}
                                editingId={editingItemId}
                                editingLabel={editingItemLabel}
                                ...
                                onCancelEdit={() => setEditingItemId(null)}
                                newLabel={newTextLabel[subtitle.id] ?? ""}
                                onNewLabelChange={(v) => setNewTextLabel((prev) => ({ ...prev, [subtitle.id]: v }))}
                                onCreate={() => handleCreateItem(subtitle.id, "TEXT")}
                              />
                            </>
                          )}
                        </div>
