...
@RestController
@RequestMapping("/api/teacher/students/{studentId}/goals")
public class TeacherGoalController {

    private final GoalService goalService;
    private final GoalProgressService goalProgressService;

    public TeacherGoalController(GoalService goalService, GoalProgressService goalProgressService) {
    ...
    public ResponseEntity<?> create(
        ...
        try {
            return ResponseEntity.status(HttpStatus.CREATED)
                    .body(goalService.create(studentId, principal.getId(), request));
        } catch (IllegalArgumentException e) {
            return ResponseEntity.badRequest().body(new ErrorResponse(e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> update(
        ...
        try {
            return ResponseEntity.ok(goalService.update(goalId, studentId, principal.getId(), request));
        } catch (IllegalArgumentException e) {
            return ResponseEntity.badRequest().body(new ErrorResponse(e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> delete(
            ...
            @PathVariable UUID goalId) {
        try {
            goalService.delete(goalId, studentId, principal.getId());
    ...
    public ResponseEntity<?> createProgress(
        ...
        try {
            return ResponseEntity.status(HttpStatus.CREATED)
                    .body(goalProgressService.create(goalId, studentId, principal.getId(), request));
        } catch (IllegalArgumentException e) {
            return ResponseEntity.badRequest().body(new ErrorResponse(e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> updateProgress(
        ...
        try {
            return ResponseEntity.ok(
                    goalProgressService.update(progressId, goalId, studentId, principal.getId(), request));
        } catch (IllegalArgumentException e) {
            return ResponseEntity.badRequest().body(new ErrorResponse(e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> deleteProgress(
            ...
            @PathVariable UUID progressId) {
        try {
            goalProgressService.delete(progressId, goalId, studentId, principal.getId());
            return ResponseEntity.noContent().build();
        } catch (IllegalArgumentException e) {
            return ResponseEntity.badRequest().body(new ErrorResponse(e.getMessage()));
        }
