...
import com.reportapp.reportappbackend.entity.ReportItemType;
...
public class ReportItemDefinitionService {
    ...
    public ReportItemDefinitionResponse create(
        ...
        ReportItemType itemType = parseItemType(request.itemType());
        ...
        definition.setDisplayOrder(reportItemDefinitionMapper.findMaxDisplayOrder(subtitleId, itemType.name()) + 1);
    ...
    public ReportItemDefinitionResponse update(
        ...
        definition.setItemType(parseItemType(request.itemType()));
        definition.setLabel(request.label());
    ...
    public void reorder(
        ...
        for (UUID definitionId : request.orderedIds()) {
            ...
            if (definition.getItemType() != itemType) {
                throw new IllegalArgumentException("itemTypeが一致しません");
            }
        }
        int order = 0;
        for (UUID definitionId : request.orderedIds()) {
    ...
    private static ReportItemType parseItemType(String itemType) {
        try {
            return ReportItemType.valueOf(itemType);
        } catch (IllegalArgumentException e) {
            throw new IllegalArgumentException("不正なitemTypeです");
        }
    }
