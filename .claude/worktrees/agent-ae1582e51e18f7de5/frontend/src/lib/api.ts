import type {
  ...
  ReportImageResponse,
  ReportItemDefinitionCreateRequest,
  ReportItemDefinitionResponse,
  ReportItemDefinitionUpdateRequest,
...
async function ensureCsrfCookie(): Promise<void> {
  ...
  if (!csrfEnsured) {
    csrfEnsured = fetch(`${API_BASE_URL}/api/auth/csrf`, {
      credentials: "include",
    }).then(() => undefined);
  }
  await csrfEnsured;
...
export const studentApi = {
  ...
  listItemDefinitions(reportDate: string) {
    return request<StudentReportItemResponse[]>(
...
export const teacherApi = {
  ...
  publishBlog(blogId: string) {
    return request<BlogResponse>(`/api/teacher/blogs/${blogId}/publish`, { method: "POST" });
  },
...
function makeReportItemGroupsApi(basePath: (studentId: string) => string) {
  ...
  return {
    ...
    createItem(
      ...
      groupId: string,
      subtitleId: string,
      req: ReportItemDefinitionCreateRequest
    ) {
      return request<ReportItemDefinitionResponse>(itemPath(studentId, groupId, subtitleId), {
        method: "POST",
        body: req,
    ...
    updateItem(
      ...
      subtitleId: string,
      itemId: string,
      req: ReportItemDefinitionUpdateRequest
    ) {
