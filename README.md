...
- 日報提出機能
    ...
    - 日報は、チェック項目と自由記述欄がある
        - チェック項目の個数や内容は、担当講師（または管理者）が目標ごとに設定する。日報本文の自由記述は別途入力できる
        - 項目は「目標 → 項目グループ → サブタイトル → チェック項目」という階層で管理され、生徒ごと・目標ごとに異なる項目の日報内容となる（個別最適化ができている）
    - その他項目
        - 教科ごとの勉強時間
    - 画像の添付も可能
...
- 講師ホーム・日報フィード
    - 講師画面はInstagram風のモバイルファーストUIで、PCでも幅480pxのキャンバスとして表示する
    - ホームには全生徒の提出済み日報を新着順のフィードで表示し、担当生徒に絞り込んで確認することもできる
    - 生徒が提出した日報の内容を閲覧・コメント・いいねなどができる
    - 全生徒に対して閲覧・コメント・いいねを実行できる（生徒は自分の日報を閲覧・いいねできる）
- ブログ機能
    - 全講師の公開済みブログをフィードで閲覧できる
    - 自分のブログを新規作成・投稿できる
    - 生徒へのメッセージや日記などを投稿し、画像の添付も可能
    - 生徒や他の講師からのコメントに返信したり、いいねをしたりできる
- 生徒管理・設定機能
    - 管理者が生徒ごとに担当講師を設定する
    - 講師は、自分の担当生徒の一覧や学習状況（連続提出日数など）を確認できる
    - 担当生徒ごとに、目標（開始日・終了日を持つ期間付き）や進捗を設定・管理できる（同一生徒内で期間が重複する目標は登録できない）
    - 目標ごとに日報項目（項目グループ・サブタイトル・チェック項目、並び替え含む）を設定できる
    - 講師プロフィール画像を設定画面からアップロード・削除できる
- リアルタイム更新
    - 講師の日報・ブログのコメント・いいね機能は維持し、コメントと日報提出通知は従来どおりWebSocketで画面に反映する

## 管理者向け機能
- ログイン・ログアウト
    - 登録されたアカウント情報でログインができる
    - 自分でアカウントを作成することはできない
    - 初期データとして管理者アカウント1件をマイグレーションで投入済み（`admin` / `admin1234`）
- 講師登録
    - 講師アカウントの発行
    - 担当生徒のアサインも実行できる
- 生徒登録
...
### 設計方針
- **チェック項目はcheckboxのみ**：記述項目定義と回答欄はV14で削除し、日報本体の`freeText`（`daily_reports.free_text`）は独立して保持する。目標はサブタイトル5個まで、各サブタイトルのチェック項目5個まで。編集時はラベル・期間のみ変更でき、構造の追加・削除・移動は拒否する。項目0件の目標期間も有効で、期間内の日報は自由記述・学習時間・画像だけで提出できる。
- **提出済み日報の期間保護**：目標期間の更新では、旧期間内かつ`submitted_at IS NOT NULL`の日報の最古/最新日を境界として使用する。同日境界は許可し、目標削除は提出済み日報がある場合に拒否する。日報に`goal_id`がないため旧期間の範囲検索で判定する。
...
- **目標駆動の日報項目階層**：日報項目は生徒に直接紐づくのではなく、「目標(`goals`) → 項目グループ(`report_item_groups`) → サブタイトル(`report_item_subtitles`) → チェック項目定義(`report_item_definitions`)」という階層で管理する。日報本文の自由記述は`daily_reports.free_text`に保持する。1目標につき項目グループは最大1件（`report_item_groups.goal_id`にUNIQUE制約）。
...
| GET | `/api/student/report-item-definitions` | 自分に設定されたチェック項目一覧。`{goalPeriodExists, items}`を返し、項目0件でも目標期間内か判定できる |
...
| GET | `/api/teacher/students/{studentId}/goal-forms/{goalId}` | 目標と日報項目をまとめた編集フォーム取得 |
| POST | `/api/teacher/students/{studentId}/goal-forms` | 目標と日報項目をまとめて作成 |
| PUT | `/api/teacher/students/{studentId}/goal-forms/{goalId}` | 目標と日報項目をまとめて更新 |
...
│   │   ├── goals/                # 目標一覧から遷移する新規登録（new）・編集（[goalId]/edit）
...
│   ├── ReportItemsManager.tsx  # 管理者向け日報項目設定UI
│   ├── TeacherGoalForm.tsx      # 講師向け目標・チェック項目の統合フォーム
│   ├── TeacherReportCard.tsx    # 講師向け日報カード
...
- 生徒画面と講師画面はInstagram風のフィードUI（`ig-`プレフィックスのクラス群：`ig-card`, `ig-story-*`, `ig-action-bar`, `ig-comment-*`等）を採用し、講師画面もPC上で幅480pxのモバイルキャンバスとして表示する。講師ホームは全生徒の日報フィード、講師ブログは全講師の公開済み投稿フィードを中心に構成する。管理者向けページは共通クラスを使った管理画面のスタイルとする。
