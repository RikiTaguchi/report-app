...
public class BlogService {
    ...
    public List<Blog> listEntitiesByTeacherId(UUID teacherId) {
        return blogMapper.findByTeacherId(teacherId);
    }
    ...
    public List<BlogResponse> listAll(ActorType actorType, UUID actorId) {
        return blogMapper.findAll().stream().map(blog -> toResponse(blog, actorType, actorId)).toList();
    }
    ...
    public boolean isVisibleTo(UUID blogId, String role, UUID actorId) {
        return blogMapper.findById(blogId).isPresent();
    }
