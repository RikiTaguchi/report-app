...
public class FileController {
    ...
    public ResponseEntity<Resource> get(
        ...
        Optional<Student> studentProfileOwner = studentMapper.findByProfileImageUrl(imageUrl);
        if (studentProfileOwner.isPresent()) {
            if (!isStudentProfileImageVisibleTo(studentProfileOwner.get(), principal)) {
                return ResponseEntity.status(403).build();
            }
            return serve(filename);
        }

        Optional<Teacher> teacherProfileOwner = teacherMapper.findByProfileImageUrl(imageUrl);
        if (teacherProfileOwner.isPresent()) {
            if (!isTeacherProfileImageVisibleTo(teacherProfileOwner.get(), principal)) {
                return ResponseEntity.status(403).build();
            }
            return serve(filename);
        }
