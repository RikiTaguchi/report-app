...
export function ProfileImageModal({ currentImageUrl, name, onClose, onUpdated }: ProfileImageModalProps) {
 ...
 const handleSave = async () => {
   ...
   setSubmitting(true);
   try {
     const res = await teacherApi.uploadProfileImage(file);
 ...
 const handleDelete = async () => {
   setSubmitting(true);
   try {
     const res = await teacherApi.deleteProfileImage();
     onUpdated(res.profileImageUrl);
 ...
 return (
   <div className="ig-modal-backdrop" onClick={onClose}>
     <div className="ig-modal-panel" onClick={(e) => e.stopPropagation()}>
       ...
       <input type="file" accept="image/*" onChange={handleFileChange} disabled={submitting || converting} />
       ...
       <div className="ig-modal-actions">
         <button
           ...
           onClick={handleSave}
           disabled={!file || submitting || converting}
         >
           {submitting ? "保存中..." : "保存"}
         </button>
         {currentImageUrl && (
