...
export interface TeacherBlogFormImage {
 key: string;
 url: string;
 /** true の場合、URL に関係なくスピナータイルとして描画する */
 loading?: boolean;
 /** true の場合、画像の上にスピナーオーバーレイを重ねて表示する */
       ...
       <div className="ig-settings-section">
         ...
         <div className="image-row">
           {images.map((image, index) => (
             <div
               key={image.key}
               className={`image-tile${image.loading ? " image-tile-placeholder" : ""}`}
             >
               {image.loading ? (
                 <div className="spinner" aria-label="処理中" />
               ) : (
                 <>
                   <img src={image.url} alt={`プレビュー ${index + 1}`} />
                   {image.overlayLoading && (
